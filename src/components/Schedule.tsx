...
const SWATCH_BG: Record<string, string> = {
  ...
  rose: 'bg-rose-400', purple: 'bg-violet-400', sky: 'bg-sky-400',
};
...
function snapToQuarter(mins: number) { return Math.round(mins / 15) * 15; }
...
export default function Schedule({ blocks, onAdd, onUpdate, onDelete, tasks = [], onAddTask, onToggleTask, onDeleteTask }: Props) {
  ...
  const allBlocks = useMemo(() => {
    ...
    const visibleGoogle = googleEvents.filter(
      (e) => !e.googleCalendarId || !hiddenCalendars.has(e.googleCalendarId)
    );
  ...
  function handleDragStart(e: React.MouseEvent<HTMLDivElement>, date: Date) {
    ...
    dragRef.current = { day: dayNum, startMins: 0, active: true, date: toISODate(date) };
    const mins = yToMinutes(e.clientY);
  ...
  const [resizing, setResizing] = useState<{ id: string; edge: 'top' | 'bottom'; originalStart: number; originalEnd: number } | null>(null);
  const [resizeMins, setResizeMins] = useState(0);
  const resizeRef = useRef<{ id: string; edge: 'top' | 'bottom'; originalStart: number; originalEnd: number } | null>(null);

  function handleResizeStart(e: React.MouseEvent, block: TimeBlock, edge: 'top' | 'bottom') {
    e.stopPropagation();
    e.preventDefault();
    const info = { id: block.id, edge, originalStart: toMinutes(block.startTime), originalEnd: toMinutes(block.endTime) };
    resizeRef.current = info;
    dragRef.current = { day: new Date(block.date + 'T00:00').getDay(), startMins: 0, active: false, date: block.date };
  ...
  const [moving, setMoving] = useState<{ id: string; duration: number; originalDate: string; originalStart: number } | null>(null);
  const [moveMins, setMoveMins] = useState(0);
  const [moveDate, setMoveDate] = useState('');
  const moveRef = useRef<{ id: string; duration: number; offsetMins: number; originalDate: string; originalStart: number } | null>(null);
  ...
  function handleMoveStart(e: React.MouseEvent, block: TimeBlock) {
    ...
    dragRef.current = { day: dayNum, startMins: 0, active: false, date: block.date };
    const clickMins = yToMinutes(e.clientY);
    const offsetMins = clickMins - startMins;
    moveRef.current = { id: block.id, duration, offsetMins, originalDate: block.date, originalStart: startMins };
    setMoveMins(startMins);
    setMoveDate(block.date);
    setMoving({ id: block.id, duration, originalDate: block.date, originalStart: startMins });
  }
  ...
  const handleMoveMove = useCallback((e: MouseEvent) => {
    ...
    const mins = yToMinutes(e.clientY);
    const newStart = Math.max(START_HOUR * 60, Math.min(END_HOUR * 60 - moveRef.current.duration, mins - moveRef.current.offsetMins));
  ...
  return (
    ...
    <div ref={containerRef} className="relative flex flex-col gap-3 flex-1 min-w-0">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        {googleConnected ? (
          <span className="text-[10px] text-sky-500 dark:text-sky-400 font-medium flex items-center gap-1" title="Google Calendar connected">
        ...
        {view === 'week' && (
          <div className="flex items-center gap-2">
            ...
            >
              {weekOffset === 0 ? 'This Week' : `${weekDates[0].toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} – ${weekDates[6].toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}`}
            </button>
            <button
              onClick={() => setWeekOffset((o) => o + 1)}
              className="w-6 h-6 flex items-center justify-center rounded-md text-stone-400 dark:text-stone-600 hover:text-stone-700 dark:hover:text-stone-300 hover:bg-stone-100 dark:hover:bg-stone-800 transition-all text-sm"
          ...
          >
            <div className="relative flex" style={{ height: `${HOURS.length * HOUR_HEIGHT}px` }}>
              ...
              <div className="w-10 shrink-0 relative select-none">
                {HOURS.map((h, idx) => (
                  <div key={h} style={{ position: 'absolute', top: `${idx * HOUR_HEIGHT - 7}px`, right: '6px' }}>
                ...
                return (
                  ...
                  >
                    ...
                    {HOURS.map((_, idx) => (
                      <div
                        key={`h${idx}`}
                        style={{ position: 'absolute', top: `${idx * HOUR_HEIGHT + HOUR_HEIGHT / 2}px`, left: 0, right: 0, height: '1px' }}
                    ...
                    {isToday && showNow && (
                      <div
                        style={{ position: 'absolute', top: `${nowTop}px`, left: 0, right: 0, zIndex: 10 }}
                    ...
                    {dayBlocks.map((block) => {
                      ...
                      return (
                        <div
                          ...
                          style={{ position: 'absolute', top, height, left: 2, right: 2, zIndex: isSelected ? 25 : isResizing || isMoving ? 20 : 5, ...hexColorStyle(block.color) }}
                        ...
                        >
                          ...
                          >
                            <div className="text-[11px] font-semibold truncate leading-snug">
                              {isGoogle && <span className="opacity-50 mr-0.5" title="Google Calendar">G</span>}
                              {block.title}
                            </div>
                            {height > 28 && (
                              <div className="text-[10px] opacity-60 font-mono leading-none mt-0.5">
                                {formatTime(`${pad(Math.floor(startMins / 60))}:${pad(startMins % 60)}`)}
                                {(isResizing || isMoving) && ` – ${formatTime(`${pad(Math.floor(endMins / 60))}:${pad(endMins % 60)}`)}`}
                              </div>
                            )}
                          </div>
                    ...
                    {moving && moveDate === toISODate(date) && !dayBlocks.some((b) => b.id === moving.id) && (() => {
                      ...
                      return (
                        <div
                          style={{ position: 'absolute', top: mTop, height: mHeight, left: 2, right: 2, zIndex: 20, ...hexColorStyle(movedBlock.color) }}
                        ...
                        >
                          <div className="text-[11px] font-semibold truncate leading-snug mt-1">{movedBlock.title}</div>
                          <div className="text-[10px] opacity-60 font-mono leading-none mt-0.5">
                            {formatTime(`${pad(Math.floor(moveMins / 60))}:${pad(moveMins % 60)}`)}
                            {' – '}
                            {formatTime(`${pad(Math.floor((moveMins + moving.duration) / 60))}:${pad((moveMins + moving.duration) % 60)}`)}
                          </div>
                        </div>
                      );
                    })()}
                    ...
                    {isDragTarget && (dragging || adding) && (
                      <div
                        ...
                        style={{ position: 'absolute', top: dragTop, height: dragHeight, left: 2, right: 2, zIndex: 20 }}
                      ...
                      >
                        ...
                        {adding && title && (
                          <div className="text-[11px] font-semibold px-1.5 truncate">{title}</div>
                        )}
                      </div>
                    )}
                  </div>
      ...
      {!adding && (
        <p className="text-xs text-stone-400 dark:text-stone-600">
          Drag on the calendar to create a block
        </p>
      )}
      ...
      {adding && popoverPos && (
        ...
        >
          ...
          <div className="text-[11px] text-stone-500 dark:text-stone-400 font-mono">
            {new Date(formDate + 'T00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
            {'  '}
      ...
      {selectedBlock && detailPos && (
        ...
        >
          ...
          <div className="text-[11px] text-stone-500 dark:text-stone-400 font-mono">
            {new Date(selectedBlock.date + 'T00:00').toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}
            {'  '}
          ...
          {googleConnected && googleCalendars.length > 0 && (
            <select
              ...
              onChange={(e) => {
                ...
                // Update local state
                const updates = { googleCalendarId: newCalId, color: newCal.color };
          ...
          <div className="flex items-center gap-2 pt-1 border-t border-stone-100/50 dark:border-stone-800/30">
            ...
            <button
              onClick={() => { handleDeleteBlock(selectedBlock.id); closeDetail(); }}
            ...
            <button
              ...
              className={`flex items-center gap-2 text-[11px] py-1 px-1.5 rounded-md transition-all text-left ${
                hidden
                  ...
                  : 'opacity-100 hover:bg-stone-100 dark:hover:bg-stone-800/40'
              }`}
