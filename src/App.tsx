...
export default function App() {
  ...
  const [goals, setGoals] = useLocalStorage<Goal[]>('wf-goals', []);
  const [blocks, setBlocks] = useLocalStorage<TimeBlock[]>('wf-blocks', []);
  ...
  function addGoal(g: Omit<Goal, 'id' | 'createdAt'>) {
    setGoals((prev) => [...prev, { ...g, id: uid(), createdAt: Date.now() }]);
  }
  ...
  function addBlock(b: Omit<TimeBlock, 'id'>) {
    setBlocks((prev) => [...prev, { ...b, id: uid() }]);
  }
  ...
  return (
    <div className={theme === 'dark' ? 'dark' : ''}>
      <div className={`min-h-screen bg-[#faf7f2] text-stone-700 dark:bg-[#0f0e0c] dark:text-stone-300 transition-all duration-300 ${checkinOpen ? 'mr-[420px]' : ''}`}>
        ...
        {/* Header */}
        <header className="border-b border-stone-200 dark:border-stone-800/60 px-6 py-4 transition-colors duration-300">
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 border border-amber-200/70 dark:border-amber-500/20 hover:bg-amber-100 dark:hover:bg-amber-500/20 transition-all"
              >
        ...
        <div className="border-b border-amber-200/50 dark:border-amber-900/25 px-6 py-5 bg-amber-50/70 dark:bg-amber-950/20 transition-colors duration-300">
        ...
        <main className="max-w-7xl mx-auto px-6 py-6">
          <div className="grid gap-6" style={{ gridTemplateColumns: '300px 1fr' }}>
            ...
            <div className="flex flex-col gap-6">
              <div className="rounded-2xl border border-stone-200 dark:border-stone-800/60 bg-white/80 dark:bg-stone-900/20 p-5 transition-colors duration-300">
                <Section title="To-Do" subtitle="What needs to get done today">
                  <TodoList tasks={tasks} onAdd={addTask} onToggle={toggleTask} onDelete={deleteTask} />
                </Section>
              </div>
              <div className="rounded-2xl border border-stone-200 dark:border-stone-800/60 bg-white/80 dark:bg-stone-900/20 p-5 transition-colors duration-300">
            ...
            <div className="rounded-2xl border border-stone-200 dark:border-stone-800/60 bg-white/80 dark:bg-stone-900/20 p-5 transition-colors duration-300">
              <Section title="This Week" subtitle="Click any slot to add a block">
                <Schedule blocks={blocks} onAdd={addBlock} onUpdate={updateBlock} onDelete={deleteBlock} />
              </Section>
            </div>
          </div>
        </main>
